...
int main ( void )
{
  ...
  cout << "SPLINE_PRB\n";
  cout << "  C++ version:\n";
  cout << "  Test the routines in the SPLINE library.\n";
...
# define MAXDATA 10
  ...
  cout << "\n";
  cout << "TEST12\n";
  cout << "  BP_APPROX evaluates the Bernstein polynomial\n";
  ...
  for ( ndata = 0; ndata <= 9; ndata = ndata + 3 )
  {
    ...
    for ( i = 1; i<= nsample; i++ )
    {
      ...
      yval = bp_approx ( ndata, a, b, ydata, xval );
...
//  Sample the spline.
//
  t_test = r8vec_uniform ( N_TEST, tlo-1.0, thi+1.0, &seed );
  ...
  for ( k = 0; k < 3; k++ )
  {
    ...
    for ( i = 0; i <= N; i++ )
    {
      ...
      for ( j = 1; j <= jhi; j++ )
      {
        ...
        yval = spline_cubic_val ( N, t, tval, y, ypp, &ypval, &yppval );
