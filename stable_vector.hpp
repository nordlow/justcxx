...
public:
  template<typename T, typename Value>
  static typename iterator<T, Value>::nodeType* get(iterator<T, Value> const& it)
  {
    return it.pn;
  }
};
...
{
  typedef node_type<T> nodeType;

public:
  iterator(){}
  explicit iterator(nodeType* pn):pn(pn){}
  friend class node_access;
  iterator(const iterator<T, T>& x):pn(node_access::get(x)){}

private:
  static nodeType* node_ptr(void* p){return static_cast<nodeType*>(p);}
...
class stable_vector
{
  typedef stable_vector_detail::node_type<T>        nodeType;
  typedef std::vector<
    void*,
    typename Allocator::
...
private:
  static nodeType* node_ptr(void* p)
  {
  ...
  typename allocator_type::
    template rebind<nodeType>::other al;
  impl_type                           impl;
};
