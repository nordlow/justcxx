...
template <class T>
inline pure T min_of() { return std::numeric_limits<T>::min(); }
/*! Get \p T's \em maximum value. */
template <class T>
inline pure T max_of() { return std::numeric_limits<T>::max(); }
/*! Set argument to its minimum value. */
template <class T>
inline void minimize(T& a) { a = min_of<T>(); }
/*! Set argument to its maximum value. */
template <class T>
inline void maximize(T& a) { a = max_of<T>(); }
/// \}

/*! @name Check whether \p a is Minimum/Maximum of \p a. */
/// \{
template <class T>
inline pure bool is_min(const T& a) { return a == pnw::min_of<T>(); }
template <class T>
inline pure bool is_max(const T& a) { return a == pnw::max_of<T>(); }
/// \}

/// @name Return Uninitialized Value of Type \c T.
/// @{
template <class T>
inline T uninitialized() { return std::numeric_limits<T>::max(); }
template <>
inline float uninitialized<float>() { return nanf("NAN(string)"); }
template <>
inline double uninitialized<double>() { return nan("NAN(string)"); }
template <>
inline long double uninitialized<long double>() { return nanl("NAN(string)"); }
...
template <class T>
inline pure typename std::enable_if<std::is_integral<T>::value, bool>::type is_defined(T a) { return a != 0; }
template <class T>
inline pure typename std::enable_if<std::is_pointer<T>::value, bool>::type is_defined(T a) { return a != nullptr; }
template <class T>
inline pure typename std::enable_if<std::is_floating_point<T>::value, bool>::type is_defined(T a) { return a != 0; }
// inline pure bool is_defined(const std::string&  a) { return !a.empty(); }
// inline pure bool is_defined(const std::string&& a) { return !a.empty(); }
template <class C>
inline pure typename std::enable_if<is_container<C>::value, bool>::type is_defined(const C& a) { return !a.empty(); }
/// @}
...
/// @{
template <class T>
inline pure T any(T a) { return a; } // \note Or use pnw::is_defined(a) ? a : T()
template <class T, class... U>
inline pure typename std::common_type<T, U...>::type any(T a, U... b) { return pnw::is_defined(a) ? a : any(b...); }
/// @}
...
/// @{
template <class T>
inline pure T all(T a) { return a; } // \note Or use pnw::is_defined(a) ? a : T()
template <class T, class... U>
inline pure typename std::common_type<T, U...>::type all(T a, U... b) { return pnw::is_defined(a) ? all(b...) : a; } // \note Or T() instead a.
/// @}
...
template <class C>
inline pure typename std::enable_if<is_container<C>::value, C>::type::const_iterator min_element(const C& c) { return std::min_element(begin(c), end(c)); }
/*! Get \em Iterator to Largest Element in STL container (C) \p c. */
template <class C>
inline pure typename std::enable_if<is_container<C>::value, C>::type::const_iterator max_element(const C& c) { return std::max_element(begin(c), end(c)); }
/*! Read \em Minimum \p min and \em Maximum \p max Element of STL Container \p a. */
template <class C>
inline pure typename std::pair<typename C::const_iterator,
                               typename C::const_iterator>
minmax_element(const C& c)
{
    return std::minmax_element(begin(c), end(c));
}
/*! Read \em Minimum \p min and \em Maximum \p max Element Values of STL Container \p a. */
template <class C>
inline pure std::pair<typename C::value_type,
                      typename C::value_type>
minmax_element_value(const C& c)
{
...
template <FAC C, class UnaryFunction>
inline typename std::enable_if<is_container<C>::value, UnaryFunction>::type for_each(C& a, UnaryFunction func) { return std::for_each(begin(a), end(a), func); }

/// \em Fill \p a with \p value.
template <FAC C, class T>
inline C& fill(C& a, const T& value)
{
    std::fill(begin(a), end(a), value);
    return a;
}
/// \em Fill \p a with \p count number of \p value.
template <FAC C, class Size, class T>
inline void fill(C a, Size count, const T& value)
{
    std::fill(begin(a), count, value);
    return a;
}

/// \em Sort \p a.
template <RAC C>
inline typename std::enable_if<is_container<C>::value, C>::type& sort(C& a)
{
...
template <RAC C>
inline typename std::enable_if<is_container<C>::value, C>::type sort_copy(C a)
{
    std::sort(begin(a), end(a));
    return a;
}
/// \em Sort \p a.
template <RAC C, class Compare>
inline typename std::enable_if<is_container<C>::value, C>::type sort(C& a, Compare b)
{
    std::sort(begin(a), end(a), b);
    return a;
}

/// \em Stable-Sort \p a.
template <RAC C>
inline typename std::enable_if<is_container<C>::value, C>::type stable_sort(C a)
{
    std::stable_sort(begin(a), end(a));
    return a;
}
/// \em Stable-Sort \p a.
template <RAC C, class StrictWeakOrdering>
inline typename std::enable_if<is_container<C>::value, C>::type stable_sort(C a, StrictWeakOrdering b)
{
    std::stable_sort(begin(a), end(a), b);
    return a;
}

/// \em Merge \p a and \p b into \p c.
template <class IC, class OC>
inline void merge(const IC& a, const IC& b, const OC& c)
{
...
/// \em Merge \p a and \p b into \p c.
template <class IC, class OC, class StrictWeakOrdering>
inline OC& merge(const IC& a, const IC& b, const OC& c, StrictWeakOrdering d)
{
    std::merge(begin(a), end(a),
               begin(b), end(b),
               begin(c),
...
template <class IC, class OC>
inline typename std::enable_if<is_container<OC>::value, OC>::type& copy(const IC& a, OC& b) { return b = a; }
...
template <FAC C>
inline typename std::enable_if<is_container<C>::value, bool>::type is_sorted(const C& a) { return is_sorted(begin(a), end(a)); }
/// Check if \p a is \em sorted.
template <FAC C, class Compare>
inline typename std::enable_if<is_container<C>::value, bool>::type is_sorted(const C& a, Compare comp) { return is_sorted(begin(a), end(a), comp); }

#ifdef HAVE_CXX11_LAMBDA_EXPRESSIONS
template <RAC C>
inline C& abs_sort(C& a)
{
    typedef typename C::value_type T;
    std::sort(begin(a), end(a), [](const T& x, const T& y) { return (std::abs(x) < std::abs(y)); });
    return a;
}
/// Sort \p a on the absolute values of the elements.
template <RAC C>
inline C& abs_stable_sort(C& a)
{
    typedef typename C::value_type T;
    std::stable_sort(begin(a), end(a), [](const T& x, const T& y) { return (std::abs(x) < std::abs(y)); });
    return a;
}
#endif
...
template <class II>
inline typename II::value_type random_pick(II first, II last)
{
    typename II::value_type pick;
    __gnu_cxx::random_sample(first, last, (&pick) + 0, (&pick) + 1);
    return pick;
}
// Random Pick Element in \p a.
template <FAC C>
inline typename std::enable_if<is_container<C>::value, C>::type::value_type random_pick(C a) { return random_pick(begin(a), end(a)); }

/*! Random Shuffle.
 * \see http://en.cppreference.com/w/cpp/algorithm/random_shuffle
 */
template <RAC C>
typename std::enable_if<is_container<C>::value, C>::type& random_shuffle(C& a) { std::random_shuffle(begin(a), end(a)); }

// All Of
template <class IC, class UnaryPredicate>
inline typename std::enable_if<is_container<IC>::value, bool>::type all_of(const IC& a, UnaryPredicate p) { return all_of(begin(a), end(a), p); }
template <class IC>
inline typename std::enable_if<is_container<IC>::value, bool>::type all_of(const IC& a)
{
    return all_of(begin(a), end(a), [](bool x) { return x; });
}
// Any Of
template <class IC, class UnaryPredicate>
inline typename std::enable_if<is_container<IC>::value, bool>::type any_of(const IC& a, UnaryPredicate p) { return any_of(begin(a), end(a), p); }
template <class IC>
inline typename std::enable_if<is_container<IC>::value, bool>::type any_of(const IC& a)
{
    return any_of(begin(a), end(a), [](bool x) { return x; });
}
// None Of
template <class IC, class UnaryPredicate>
inline typename std::enable_if<is_container<IC>::value, bool>::type none_of(const IC& a, UnaryPredicate p) { return none_of(begin(a), end(a), p); }
template <class IC>
inline typename std::enable_if<is_container<IC>::value, bool>::type none_of(const IC& a)
{
    return none_of(begin(a), end(a), [](bool x) { return x; });
}
...
template <class C, class BinaryPredicate>
inline C unique(C a, BinaryPredicate p)
{
    unique(begin(a), end(a), p);
    return std::move(a);
}
...
typename IC::iterator find(IC c, const T& value) { return find(begin(c), end(c), value); }
template <class IC, class UnaryPredicate>
typename IC::iterator find_if(IC c, UnaryPredicate p) { return find_if(begin(c), end(c), p); }
template <class IC, class UnaryPredicate>
...
T transformed_accumulate(II first, II last, UnaryOp transform_op, T init)
{
    return std::accumulate(first, last, init,
                           [transform_op](const T& a, const T& b) { return a + transform_op(b); });
}
/*! Transformed Accumulate.
 * \see stackoverflow.com/questions/10592193/transform-and-accumulate
 * \alias zip_with
...
{
    return std::accumulate(first, last, init,
                           [transform_op, accumulate_op](const T& a, const T& b) { return accumulate_op(a, transform_op(b)); });
}

template <class T>
void add(const T& a, const T& b) { return a + b; }
template <class T>
void sub(const T& a, const T& b) { return a - b; }
template <class T>
void mul(const T& a, const T& b) { return a * b; }
template <class T>
void div(const T& a, const T& b) { return a / b; }

template <class C, class Gen>
inline typename std::enable_if<is_container<C>::value, C>::type generate(C& c, Gen gen)
{
    generate(begin(c), end(c), gen);
    return c;
}
template <class C, class Gen>
inline typename std::enable_if<is_container<C>::value, C>::type generate_n(C& c, size_t n, Gen gen)
{
    generate_n(begin(c), n, gen);
    return c;
}
...
/// Return true if odd.
template <class T>
inline spure bool odd(const T& a) { return a % 2 == 1; }
/// Return true if even.
template <class T>
inline spure bool even(const T& a) { return a % 2 == 0; }
/// \}
...
template <class T>
inline spure T median(T a, T b, T c)
{
    return (a < b ? (b < c ? b : a < c ? c : a) : (b > c ? b : a > c ? c : a));
}
/*! Median Element Value between [\p first \p last[. */
template <class It>
inline pure typename It::value_type median_element(It first, It last)
{
    ...
    {
        std::nth_element(first, nth - 1, last);
        auto a = *(nth - 1);
        std::nth_element(first, nth + 0, last);
        auto b = *(nth + 0);
        return (a + b) / 2;
    }
}
...
template <RAC C>
inline pure typename C::value_type median_element(C c)
{
    return median_element(begin(c), end(c)); // \note Move Semantics enabled!
}
...
inline pure typename It::value_type median_element(It first, It last, Compare comp)
{
    ...
    {
        std::nth_element(first, nth - 1, last, comp);
        auto a = *(nth - 1);
        std::nth_element(first, nth + 0, last, comp);
        auto b = *(nth + 0);
        return (a + b) / 2;
    }
}
...
template <RAC C, class Compare>
inline pure typename C::value_type median_element(C c, Compare comp) { return median_element(begin(c), end(c), comp); } // \note Move Semantics enabled!
/// \}
...
template <LTC T>
inline pure T min(const T& a, const T& b) { return std::min(a, b); }
/*! Minimum of \p a, \p b and \p c. */
template <LTC T>
inline pure T min(const T& a, const T& b, const T& c) { return std::min(a, std::min(b, c)); }
/*! Minimum of \p a, \p b, \p c and \p d. */
template <LTC T>
inline pure T min(const T& a, const T& b, const T& c, const T& d) { return std::min(a, b, std::min(c, d)); }
#else
...
template <LTC T>
inline pure T max(const T& a, const T& b) { return std::max(a, b); }
/*! Maximum of \p a, \p b and \p c. */
template <LTC T>
inline pure T max(const T& a, const T& b, const T& c) { return std::max(a, std::max(b, c)); }
/*! Maximum of \p a, \p b, \p c and \p d. */
template <LTC T>
inline pure T max(const T& a, const T& b, const T& c, const T& d) { return std::max(a, b, std::max(c, d)); }
#else
...
/// \{
template <class T>
T min_element(const std::pair<T, T>& a) { return std::min<T>(a.first, a.second); }
template <class T>
T max_element(const std::pair<T, T>& a) { return std::max<T>(a.first, a.second); }
/// \}

/*! @name Heterogeneous Pair Minimum/Maximum. */
/// \{
template <class T, class U>
typename std::common_type<T, U>::type min_element(const std::pair<T, U>& a) { return common_type_min<T, U>(a.first, a.second); }
template <class T, class U>
typename std::common_type<T, U>::type max_element(const std::pair<T, U>& a) { return common_type_max<T, U>(a.first, a.second); }
/// \}
 ...
 */
template <LTC T>
inline void min_sort3(T& a, T& b, T& c)
{
    sort2(a, b);
    sort2(a, c);
}
 ...
 */
template <LTC T>
inline void max_sort3(T& a, T& b, T& c)
{
    sort2(b, c);
    sort2(a, c);
}
...
/// \{
template <LTC T>
inline void minmax3(T& a, T& b, T& c)
{
    max_sort3(a, b, c);
    sort2(a, b);
}
template <LTC T>
inline void minmax4(T& a, T& b, T& c, T& d)
{
    sort2(a, b);
    sort2(c, d);
    sort2(a, c);
...
template <LTC T>
inline void minmax5(T& a, T& b, T& c, T& d, T& e)
{
    sort2(a, b);
    sort2(c, d);
    min_sort3(a, c, e);
...
template <LTC T>
inline void minmax6(T& a, T& b, T& c, T& d, T& e, T& f)
{
    sort2(a, d);
    sort2(b, e);
    sort2(c, f);
 ...
 */
template <class T>
inline T tv_area(T d, T r0, T r1) { return square(d) * (r0 * r1) / (square(r0) + square(r1)); }

/*! First primes from 0 to 39.
 * See: "Matematiklexikon", Page 26
 */
template <class T>
inline T consecutive_primes_0_39(T a)
{
    a += 40;
    return a * a - 79 * a + 1601;
}

/*! Delete all elements of \p a and clear it. */
template <class T>
inline void delete_all_and_clear(T& a)
{
    for (auto& e : a)
    {
        delete e;
    } // for (auto i = begin(a); i != end(a); i++) { delete *i; }
    a.clear();
}
...
inline void iter_mul(Ta& y, Ta x, Tb a)
{
    y = 0;
    while (a > 0)
    {
...
/*! Default multiply */
template <class Ta, class Tb>
inline void multiply(Ta& y, Ta x, Tb a) { iter_mul(y, x, a); }

/*! Check whether \p a lacks element \p b or not. */
template <class C>
inline bool lacks(const C& a, const typename C::value_type& b)
{
    return std::find(begin(a), end(a), b) == end(a);
}
/*! Check whether \p a contains element \p b or not. */
template <class C>
inline bool has(const C& a, const typename C::value_type& b) { return !has(a, b); }

/*! Reorder/Permutate in \p a trough permutation indexes \p i into \p b. */
template <class V, class I>
void indexed_reorder(const V& a, V& b, const I& i)
{
 ...
 */
template <class T>
inline pure bool is_continuous_permutation(const T* a, size_t n)
{
...
template <class C>
inline pure bool is_continuous_permutation(const C& a)
{
...
void in_place_indexed_reorder(V& a, const P& i)
{
