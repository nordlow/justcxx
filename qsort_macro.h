...
#define _QS_SWAP(a, b, t) ((void)((t = *a), (*a = *b), (*b = t)))

/* Discontinue quicksort algorithm when partition gets below this size.
   This particular magic number was chosen to work best on a Sun 4/260. */
#define _QS_MAX_THRESH 4
...
#define _QS_STACK_SIZE	(8 * sizeof(unsigned))
#define _QS_PUSH(top, low, high)	\
	(((top->_lo = (low)), (top->_hi = (high)), ++top))
#define	_QS_POP(low, high, top)	\
	((--top, (low = top->_lo), (high = top->_hi)))
#define	_QS_STACK_NOT_EMPTY	(_stack < _top)
...
#define QSORT(QS_TYPE,QS_BASE,QS_NELT,QS_LT)		\
{									\
  QS_TYPE *const _base = (QS_BASE);				\
  const unsigned _elems = (QS_NELT);					\
  QS_TYPE _hold;							\
									\
  /* Don't declare two variables of type QS_TYPE in a single		\
   * statement: eg `TYPE a, b;', in case if TYPE is a pointer,		\
   * expands to `type* a, b;' wich isn't what we want.			\
   */									\
									\
  if (_elems > _QS_MAX_THRESH) {					\
    QS_TYPE *_lo = _base;						\
    QS_TYPE *_hi = _lo + _elems - 1;					\
    struct {								\
      QS_TYPE *_hi; QS_TYPE *_lo;					\
    } _stack[_QS_STACK_SIZE], *_top = _stack + 1;			\
									\
    while (_QS_STACK_NOT_EMPTY) {					\
      QS_TYPE *_left_ptr; QS_TYPE *_right_ptr;			\
									\
      /* Select median value from among LO, MID, and HI. Rearrange	\
         ...
         the while loops. */						\
									\
      QS_TYPE *_mid = _lo + ((_hi - _lo) >> 1);			\
									\
      if (QS_LT (_mid, _lo))						\
        _QS_SWAP (_mid, _lo, _hold);					\
      if (QS_LT (_hi, _mid))	{					\
        _QS_SWAP (_mid, _hi, _hold);					\
        if (QS_LT (_mid, _lo))					\
          _QS_SWAP (_mid, _lo, _hold);				\
      } 								\
									\
      ...
      do {								\
        while (QS_LT (_left_ptr, _mid))				\
         ++_left_ptr;							\
									\
        while (QS_LT (_mid, _right_ptr))				\
          --_right_ptr;							\
									\
        if (_left_ptr < _right_ptr) {					\
          _QS_SWAP (_left_ptr, _right_ptr, _hold);			\
          if (_mid == _left_ptr)					\
            _mid = _right_ptr;						\
     ...
     /* Set up pointers for next iteration.  First determine whether	\
        ...
        bounds on the stack and continue sorting the smaller one. */	\
									\
      if (_right_ptr - _lo <= _QS_MAX_THRESH) {			\
        if (_hi - _left_ptr <= _QS_MAX_THRESH)			\
          /* Ignore both small partitions. */				\
          _QS_POP (_lo, _hi, _top);					\
        else								\
          /* Ignore small left partition. */				\
          _lo = _left_ptr;						\
      }									\
      else if (_hi - _left_ptr <= _QS_MAX_THRESH)			\
        /* Ignore small right partition. */				\
        _hi = _right_ptr;						\
      else if (_right_ptr - _lo > _hi - _left_ptr) {			\
        /* Push larger left partition indices. */			\
        _QS_PUSH (_top, _lo, _right_ptr);				\
        _lo = _left_ptr;						\
      }									\
      else {								\
        /* Push larger right partition indices. */			\
        _QS_PUSH (_top, _left_ptr, _hi);				\
        _hi = _right_ptr;						\
      }									\
    }									\
  }									\
  ...
  /* Once the BASE array is partially sorted by quicksort the rest	\
     ...
     last element in the array (*not* one beyond it!). */		\
									\
  {									\
    QS_TYPE *const _end_ptr = _base + _elems - 1;			\
    QS_TYPE *_tmp_ptr = _base;					\
    register QS_TYPE *_run_ptr;					\
    QS_TYPE *_thresh;						\
									\
    _thresh = _base + _QS_MAX_THRESH;				\
    if (_thresh > _end_ptr)						\
      _thresh = _end_ptr;						\
									\
    ...
    for (_run_ptr = _tmp_ptr + 1; _run_ptr <= _thresh; ++_run_ptr)	\
      if (QS_LT (_run_ptr, _tmp_ptr))				\
        _tmp_ptr = _run_ptr;						\
									\
    if (_tmp_ptr != _base)						\
      _QS_SWAP (_tmp_ptr, _base, _hold);				\
									\
    ...
    while (++_run_ptr <= _end_ptr) {					\
      _tmp_ptr = _run_ptr - 1;						\
      while (QS_LT (_run_ptr, _tmp_ptr))				\
        --_tmp_ptr;							\
									\
      ++_tmp_ptr;							\
      if (_tmp_ptr != _run_ptr) {					\
        QS_TYPE *_trav = _run_ptr + 1;				\
