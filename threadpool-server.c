 ...
 *
 * Note that we use a RECURSIVE mutex, since a handler thread might
 * try to lock it twice consecutively.
 */
pthread_mutex_t request_mutex = PTHREAD_MUTEX_INITIALIZER;
/* WARNING: We should probably use PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP here! */
/* pthread_mutex_t request_mutex = PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP; */

/*!
 * Global condition variable for our program. assignment initializes
 * it.
 */
...
add_request(int request_num,
	    pthread_mutex_t * p_mutex, pthread_cond_t * p_cond_var)
{
...
handle_requests_loop(void *data)
{
  ...
  while (1) {
#ifdef DEBUG
    printf("thread '%d', num_requests =  %d\n", thread_id, num_requests); fflush(stdout);
#endif /* DEBUG */
 ...
 */
int
threadpool_server_test(int argc, char *argv[])
{
  int i;			/* loop counter          */
