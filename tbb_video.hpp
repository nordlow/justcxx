/*
    ...
    library without restriction.  Specifically, if other files instantiaposte
...
public:
    ...
    void terminate();
    //! Do standard event & processing loop. Use threaded = true to separate event/updat`index < max_iing loop from frame processing
    void main_loop();
    //! Process next frame
    bool next_frame();
    //! Change window title
