...
template< class K, bool bNoDuplicates = false, class Pr =
           std::less < K >, class A = std::allocator < K > >class sorted_vector
{
public:
    ...
    template< class It > sorted_vector (It first, It beyond, const Pr & pred = Pr (), const A & al = A ()) :key_compare_ (pred), vec_ (first, beyond, al) { stable_sort (); }
    ...
    assign (const_iterator first, const_iterator beyond)
    {
        vec_.assign (first, beyond);
    } void
    assign (size_type n, const K & x = K ())
    {
        vec_.assign (n, x);
    }
/*insert members*/
...
#else	/*  */
    void
    insert (const_iterator first, const_iterator beyond)
    {
        size_type n = std::distance (first, beyond);
        reserve (size () + n);
        for (; first != beyond; ++first)
    ...
    bool
    Lt_ (const Myt_ & x) const
    {
        return (std::
                lexicographical_compare (begin (), end (), x.begin (),
                                         x.end ()));
    }
    void
    swap (Myt_ & x)
    ...
    count (const K & k) const
    {
        ...
        size_type
            n = std::distance (Ans_.first, Ans_.second);
    ...
    {
        return
            std::lower_bound (begin (), end (), k, key_compare_);
    }
    ...
    {
        return
            std::upper_bound (begin (), end (), k, key_compare_);
    }
    ...
    {
        return
            std::equal_range (begin (), end (), k, key_compare_);
    }
...
/*functions for use with direct std::vector-access*/
    ...
    void
    sort ()			//restore sorted order after low level access
    {
    ...
    void
    stable_sort ()		//restore sorted order after low level access
    {
...
protected:
    ...
    {
        iterator
            front_ = vec_.begin (), out_ = vec_.end (), end_ = vec_.end ();
        bool bCopy_ = false;
