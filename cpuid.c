/*!
 * \file Intel and AMD x86 CPUID display program v 3.3 (1 Jan 2002)
 *
...
const char *Intel_feature_flags[] = {
...
void cpuid_doIntel(int maxi){
  ...
  if(maxi >= 1){
	...
	for(i=0x80000002;i<=0x80000004;i++){
	  ...
	  CPUID(i,eax,ebx,ecx,edx);
	  cpuid_printregs(eax,ebx,ecx,edx);
	}
	printf("\"\n");
      }
    }
  ...
  if(maxi >= 2){
    ...
    for(i=0;i<ntlb;i++){
      ...
      ntlb =  eax & 0xff;
      cpuid_decode_Intel_TLB(eax >> 8);
      cpuid_decode_Intel_TLB(eax >> 16);
      cpuid_decode_Intel_TLB(eax >> 24);

      if((ebx & 0x80000000) == 0){
	cpuid_decode_Intel_TLB(ebx);
	cpuid_decode_Intel_TLB(ebx >> 8);
	cpuid_decode_Intel_TLB(ebx >> 16);
	cpuid_decode_Intel_TLB(ebx >> 24);
      }
      if((ecx & 0x80000000) == 0){
	cpuid_decode_Intel_TLB(ecx);
	cpuid_decode_Intel_TLB(ecx >> 8);
	cpuid_decode_Intel_TLB(ecx >> 16);
	cpuid_decode_Intel_TLB(ecx >> 24);
      }
      if((edx & 0x80000000) == 0){
	cpuid_decode_Intel_TLB(edx);
	cpuid_decode_Intel_TLB(edx >> 8);
	cpuid_decode_Intel_TLB(edx >> 16);
	cpuid_decode_Intel_TLB(edx >> 24);
      }
    }
  }
  if(maxi >= 3){
...
void cpuid_decode_Intel_TLB(int x){
  x &= 0xff;
  if(x != 0)
    printf("%02x: ",x);
  switch(x){
...
const char *Cyrix_standard_feature_flags_5[] = {
  "FPU   Floating Point Unit",
...
const char *Cyrix_standard_feature_flags_not5[] = {
  "FPU   Floating Point Unit",
  "V86   Virtual Mode Extensions",
  "Debug Extension",
  "4MB Page Size",
...
const char *Cyrix_extended_feature_flags[] = {
  "FPU   Floating Point Unit",
  "V86   Virtual Mode Extensions",
  "Debug Extension",
  "Page Size Extensions",
...
void cpuid_doCyrix(int maxi){
  ...
  if(maxi >= 2){
    ...
    for(i=0;i<ntlb;i++){
      ...
      ntlb =  eax & 0xff;
      cpuid_decode_Cyrix_TLB(eax >> 8);
      cpuid_decode_Cyrix_TLB(eax >> 16);
      cpuid_decode_Cyrix_TLB(eax >> 24);

      /* ebx and ecx are reserved */

      if((edx & 0x80000000) == 0){
	cpuid_decode_Cyrix_TLB(edx);
	cpuid_decode_Cyrix_TLB(edx >> 8);
	cpuid_decode_Cyrix_TLB(edx >> 16);
	cpuid_decode_Cyrix_TLB(edx >> 24);
      }
    }
  }
  ...
  if(maxei >= 0x80000005){
    ...
    for(i=0;i<ntlb;i++){
      ...
      ntlb =  eax & 0xff;
      cpuid_decode_Cyrix_TLB(ebx >> 8);
      cpuid_decode_Cyrix_TLB(ebx >> 16);
      cpuid_decode_Cyrix_TLB(ebx >> 24);

      /* eax and edx are reserved */

      if((ecx & 0x80000000) == 0){
	cpuid_decode_Cyrix_TLB(ecx);
	cpuid_decode_Cyrix_TLB(ecx >> 8);
	cpuid_decode_Cyrix_TLB(ecx >> 16);
	cpuid_decode_Cyrix_TLB(ecx >> 24);
      }
    }
  }
}

void cpuid_decode_Cyrix_TLB(int x){
  switch(x & 0xff){
  case 0:
    break;
  case 0x70:
