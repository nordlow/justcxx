...
//  Parameters:
//
//    Input, Td TDATA(5), the nodes associated with the basis function.
//    The entries of TDATA are assumed to be distinct and increasing.
//
//    Input, Td TVAL, a point at which the B spline basis function is
//    to be evaluated.
//
//    Output, Td BASIS_FUNCTION_B_VAL, the value of the function at TVAL.
//
{
# define NDATA 5
...
//  Parameters:
//
//    Input, Td BETA1, the skew or bias parameter.
//    BETA1 = 1 for no skew or bias.
//
//    Input, Td BETA2, the tension parameter.
...
//    to be evaluated.
//
//    Output, Td BASIS_FUNCTION_BETA_VAL, the value of the function at TVAL.
//
{
# define NDATA 5
...
//  Parameters:
//
//    Output, Td BASIS_MATRIX_B_UNI[4*4], the basis matrix.
//
{
  int i;
  int j;
...
//    BETA2 = 0 for no tension.
//
//    Output, Td BASIS_MATRIX_BETA_UNI[4*4], the basis matrix.
//
{
  Td delta;
  int i;
...
//  Parameters:
//
//    Output, Td BASIS_MATRIX_BEZIER[4*4], the basis matrix.
//
{
  Td *mbasis;
...
//  Parameters:
//
//    Output, Td BASIS_MATRIX_HERMITE[4*4], the basis matrix.
//
{
  Td *mbasis;
...
//
//    Output, Td BASIS_MATRIX_OVERHAUSER_NONUNI[4*4], the basis matrix.
//
{
  Td *mbasis;
...
//    ALPHA = || P2 - P1 || / ( || P3 - P2 || + || P2 - P1 || )
//
//    Output, Td BASIS_MATRIX_OVERHAUSER_NUL[3*3], the basis matrix.
//
{
  Td *mbasis;
...
//
//    Output, Td BASIS_MATRIX_OVERHAUSER_NUR[3*3], the basis matrix.
//
{
  Td *mbasis;
...
//  Parameters:
//
//    Output, Td BASIS_MATRIX_OVERHASUER_UNI[4*4], the basis matrix.
//
{
  Td *mbasis;
...
//  Parameters:
//
//    Output, Td BASIS_MATRIX_OVERHASUER_UNI_L[3*3], the basis matrix.
//
{
  Td *mbasis;
...
//  Parameters:
//
//    Output, Td BASIS_MATRIX_OVERHASUER_UNI_R[3*3], the basis matrix.
//
{
  Td *mbasis;
...
//
//    Input, Td MBASIS[N*N], the basis matrix.
//
//    Input, int NDATA, the dimension of the vectors TDATA and YDATA.
//
//    Input, Td TDATA[NDATA], the abscissa values.  This routine
//    assumes that the TDATA values are uniformly spaced, with an
//    increment of 1.0.
//
//    Input, Td YDATA[NDATA], the data values to be interpolated or
//    approximated.
//
//    Input, Td TVAL, the value of T at which the spline is to be
//    evaluated.
//
//    Output, Td BASIS_MATRIX_TMP, the value of the spline at TVAL.
//
{
  Td arg;
  int first;
...
//    must be at least 0.
//
//    Input, Td T, the point at which the Bezier curve should
//    be evaluated.  The best results are obtained within the interval
//    [0,1] but T may be anywhere.
//
//    Input, Td XCON[0:N], YCON[0:N], the X and Y coordinates
...
//
//    Output, Td *XVAL, *YVAL, the X and Y coordinates of the point
...
//    must be at least 0.
//
//    Input, Td X, the point at which the Bezier function should
//    be evaluated.  The best results are obtained within the interval
//    [A,B] but X may be anywhere.
//
//    Input, Td A, B, the interval over which the Bezier function
...
//
//    Input, Td Y[0:N], a set of data defining the Y coordinates
//    of the control points.
//
//    Output, Td BEZ_VAL, the value of the Bezier function at X.
//
{
  Td *bval;
  int i;
...
//
//    Input, Td A, B, the endpoints of the interval on which the
//    approximant is based.  A and B should not be equal.
//
//    Input, Td YDATA[0:N], the data values at N+1 equally spaced points
//    in [A,B].  If N = 0, then the evaluation point should be 0.5 * ( A + B).
//    Otherwise, evaluation point I should be ( (N-I)*A + I*B ) / N ).
//
//    Input, Td XVAL, the point at which the Bernstein polynomial
//    approximant is to be evaluated.  XVAL does not have to lie in the
//    interval [A,B].
//
//    Output, Td BP_APPROX, the value of the Bernstein polynomial approximant
...
//
//    Input, Td X, the evaluation point.
//
//    Output, Td BP01[0:N], the values of the N+1 Bernstein basis
//    polynomials at X.
//
{
  Td *bern;
...
//
//    Input, Td X, the point at which the polynomials are to be
//    evaluated.  X need not lie in the interval [A,B].
//
//    Output, Td BERN[0:N], the values of the N+1 Bernstein basis
//    polynomials at X.
//
{
  Td *bern;
...
//  Parameters:
//
//    Input, Td X1, X2, the endpoints of the interval of
//    definition of the cubic.  X1 and X2 must be distinct.
//
//    Input, Td F1, F2, the values of the function at X1 and
//    X2, respectively.
//
//    Input, Td D1, D2, the derivative values at X1 and
//    X2, respectively.
//
//    Input, int NE, the number of evaluation points.
//
//    Input, Td XE[NE], the points at which the function is to
//    be evaluated.  If any of the XE are outside the interval
//    [X1,X2], a warning error is returned in NEXT.
//
//    Output, Td FE[NE], the value of the cubic function
//    at the points XE.
//
...
//
//    Input, Td A[3*N], the D3 matrix.
//
//    Input, Td X[N], the vector to be multiplied by A.
//
//    Output, Td D3_MXV[N], the product A * x.
//
{
  Td *b;
  int i;
...
//    Input, int N, the order of the linear system.
//
//    Input/output, Td A[3*N].
//    On input, the nonzero diagonals of the linear system.
//    On output, the data in these vectors has been overwritten
//    by factorization information.
//
//    Input, Td B[N], the right hand side.
//
//    Output, Td D3_NP_FS[N], the solution of the linear system.
...
//
//    Output, Td D3_UNIFORM[3*N], the D3 matrix.
//
{
  Td *a;
  int i;
...
//
//    Input, Td XTAB[NTAB], the X values at which data was taken.
//    These values must be distinct.
//
//    Input, Td YTAB[NTAB], the corresponding Y values.
//
//    Output, Td DIFTAB[NTAB], the divided difference coefficients
...
//
//    Input, Td XTAB[NTAB], the X values upon which the
//    divided difference polynomial is based.
//
//    Input, Td DIFTAB[NTAB], the divided difference table.
//
//    Input, Td XVAL, a value of X at which the polynomial
//    is to be evaluated.
//
//    Output, Td DIF_VAL, the value of the polynomial at XVAL.
//
{
  int i;
  Td value;
...
//
//    Input, Td X[POINT_NUM], the abscissas of the data points.
//    At least NTERMS of the values in X must be distinct.
//
//    Input, Td F[POINT_NUM], the data values at the points X(*).
//
//    Input, Td W[POINT_NUM], the weights associated with
...
//
//    Output, Td B[NTERMS], C[NTERMS], D[NTERMS], are quantities
...
//
//    Input, Td B[NTERMS], C[NTERMS], D[NTERMS], the information
//    computed by R8POLY_LS_SET.
//
//    Input, Td X, the point at which the least squares polynomial
//    is to be evaluated.
//
//    Output, Td LEAST_VAL, the value of the least squares
//    polynomial at X.
//
{
  int i;
...
//    is to be evaluated.
//
//    Output, Td *PX, *PXP, the value and derivative of the least
...
//
//    Input, Td XTAB[NTAB], the X data.  The values in XTAB
//    should be distinct, and in increasing order.
//
//    Input, Td YTAB[NTAB], the Y data values corresponding
...
//    equal to NTAB-1.
//
//    Output, Td PTAB[NTAB], the value of the least squares polynomial
//    at the points XTAB(1:NTAB).
//
//    Output, Td B[1:NDEG], C[0:NDEG], D[2:NDEG], arrays containing
//    data about the polynomial.
//
//    Output, Td *EPS, the root-mean-square discrepancy of the
//    polynomial fit.
//
...
//
  ...
  if ( ndeg == 0 )
  {
    *eps = 0.0;
    for ( i = 0; i < ntab; i++ )
    {
      *eps = *eps + pow ( ( y_sum / ( Td ) ( ntab ) - ytab[i] ), 2 );
    }

    *eps = sqrt ( *eps / ( Td ) ( ntab ) );
    delete [] ztab;
    return;
  }
//
...
//  Parameters:
//
//    Input, Td X, the point at which the polynomial is to be evaluated.
//
//    Input, int NDEG, the degree of the polynomial fit used.
//    This is the value of NDEG as returned from LEAST_SET_OLD.
//
//    Input, Td B[1:NDEG], C[0:NDEG], D[2:NDEG], arrays defined by
//    LEAST_SET, and needed to evaluate the polynomial.
//
//    Output, Td LEAST_VALPOLD, the value of the polynomial at X.
//
{
  int B_OFFSET = -1;
  int D_OFFSET = -2;
...
//
//    Input, Td TDATA[NDATA], the abscissas of the data points.  The
//    values in TDATA must be in strictly ascending order.
//
//    Input, Td YDATA[NDIM*NDATA], the data points corresponding to
//    the abscissas.
//
...
//
//    Input, Td TVAL, the value of T at which the parabolic interpolant
//    is to be evaluated.  Normally, TDATA[0] <= TVAL <= T[NDATA-1], and
//    the data will be interpolated.  For TVAL outside this range,
//    extrapolation will be used.
//
//    Output, Td YVAL[NDIM], the value of the parabolic interpolant
//    at TVAL.
//
{
  Td dif1;
...
//  Parameters:
//
//    Input, Td ARG1, ARG2, two values to check.
//
//    Output, Td PCHST,
...
//  Parameters:
//
//    Input, Td X, Y, the quantities to compare.
//
//    Output, Td R8_MAX, the maximum of X and Y.
//
{
  if ( y < x )
  {
...
//    Input, Td X, Y, the quantities to compare.
//
//    Output, Td R8_MIN, the minimum of X and Y.
//
{
  if ( y < x )
  {
...
//
//    Output, Td R8_UNIFORM_01, a new pseudorandom variate, strictly between
//    0 and 1.
//
{
  int k;
...
//
  r = ( Td ) ( *seed ) * 4.656612875E-10;

  return r;
}
//****************************************************************************80

void r8vec_bracket ( int n, Td x[], Td xval, int *left,
  int *right )
...
//
//    Input, Td X[N], an array that has been sorted into ascending order.
//
//    Input, Td XVAL, a value to be bracketed.
//
...
//
//    Input, Td T[N], an array that has been sorted into ascending order.
//
//    Input, Td TVAL, a value to be bracketed by entries of T.
//
...
//
//    Input, Td ALO, AHI, the low and high values.
//
//    Output, Td R8VEC_EVEN[N], N evenly spaced values.
...
//
//    Output, Td R8VEC_INDICATOR[N], the array to be initialized.
//
{
  int i;
  Td *a;
...
//
//    Input, Td X[N], the array to be checked.
//
...
//
//    Input, Td A[N], the vector to be printed.
//
...
//
//    Input, Td B, C, the lower and upper limits of the pseudorandom values.
//
//    Input/output, int *SEED, a seed for the random number generator.
//
//    Output, Td R8VEC_UNIFORM_01[N], the vector of pseudorandom values.
//
{
  int i;
  int k;
  ...
  for ( i = 0; i < n; i++ )
  {
    ...
    *seed = 16807 * ( *seed - k * 127773 ) - k * 2836;
    ...
    r[i] = b + ( c - b ) * ( Td ) ( *seed ) * 4.656612875E-10;
  }
...
//    Input, int N, the number of elements of A.
//
//    Input, Td A[N], the array to examine, which does NOT have to
//    be sorted.
//
//    Input, Td TOL, a tolerance for checking equality.
//
...
//    Input, int N, the number of entries in the vector.
//
//    Output, Td A[N], a vector of zeroes.
//
{
  int i;
...
//
//    Input, Td TDATA[NDATA], the abscissas of the data.
//
//    Input, Td YDATA[NDATA], the data values.
//
//    Input, Td TVAL, a point at which the spline is to be evaluated.
//
//    Output, Td SPLINE_B_VAL, the value of the function at TVAL.
//
{
  Td bval;
  int left;
...
//    Input, Td TVAL, a point at which the spline is to be evaluated.
//
//    Output, Td SPLINE_BETA_VAL, the value of the function at TVAL.
//
{
  Td a;
  Td b;
...
Td spline_constant_val ( int ndata, Td tdata[], Td ydata[],
  Td tval )
...
//
//    Input, Td TDATA[NDATA-1], the breakpoints.  The values of TDATA should
//    be distinct and increasing.
//
//    Input, Td YDATA[NDATA], the values of the spline in the intervals
//    defined by the breakpoints.
//
//    Input, Td TVAL, the point at which the spline is to be evaluated.
//
//    Output, Td *SPLINE_CONSTANT_VAL, the value of the spline at TVAL.
//
{
  int i;
...
Td *spline_cubic_set ( int n, Td t[], Td y[], int ibcbeg,
  Td ybcbeg, int ibcend, Td ybcend )
...
//
//    Input, Td T[N], the knot values, that is, the points were data is
//    specified.  The knot values should be distinct, and increasing.
//
//    Input, Td Y[N], the data values to be interpolated.
//
...
//
//    Input, Td YBCBEG, the values to be used in the boundary
...
//
//    Input, Td YBCEND, the values to be used in the boundary
//    conditions if IBCEND is equal to 1 or 2.
//
//    Output, Td SPLINE_CUBIC_SET[N], the second derivatives of the cubic spline.
//
{
  Td *a;
  Td *b;
...
Td spline_cubic_val ( int n, Td t[], Td tval, Td y[],
  Td ypp[], Td *ypval, Td *yppval )
...
//
//    Input, Td Y[N], the data values at the knots.
//
//    Input, Td T[N], the knot values.
//
//    Input, Td TVAL, a point, typically between T[0] and T[N-1], at
//    which the spline is to be evalulated.  If TVAL lies outside
//    this range, extrapolation is used.
//
//    Input, Td Y[N], the data values at the knots.
//
//    Input, Td YPP[N], the second derivatives of the spline at
//    the knots.
//
//    Output, Td *YPVAL, the derivative of the spline at TVAL.
//
//    Output, Td *YPPVAL, the second derivative of the spline at TVAL.
//
//    Output, Td SPLINE_VAL, the value of the spline at TVAL.
//
{
  Td dt;
  Td h;
...
//    the knots.
//
//    Output, Td *YVAL, *YPVAL, *YPPVAL, the value of the spline, and
...
//
//    Input, Td TDATA[NDATA], the abscissas of the data points.
//    The entries of TDATA are assumed to be strictly increasing.
//
//    Input, Td Y[NDATA], YP[NDATA], the value of the
//    function and its derivative at TDATA(1:NDATA).
//
//    Output, Td SPLINE_HERMITE_SET[4*NDATA], the coefficients of
...
//    The entries of TDATA are assumed to be strictly increasing.
//
//    Input, Td C[4*NDATA], the coefficient data computed by
//    SPLINE_HERMITE_SET.
//
//    Input, Td TVAL, the point where the interpolant is to
//    be evaluated.
//
//    Output, Td *SVAL, *SPVAL, the value of the interpolant
...
Td spline_linear_int ( int ndata, Td tdata[], Td ydata[],
  Td a, Td b )
...
//    Input, int NDATA, the number of data points defining the spline.
//
//    Input, Td TDATA[NDATA], YDATA[NDATA], the values of the independent
//    and dependent variables at the data points.  The values of TDATA should
//    be distinct and increasing.
//
//    Input, Td A, B, the interval over which the integral is desired.
//
//    Output, Td SPLINE_LINEAR_INT, the value of the integral.
//
{
  Td a_copy;
  int a_left;
...
void spline_linear_intset ( int n, Td int_x[], Td int_v[],
  Td data_x[], Td data_y[] )
...
//
//    Input, Td INT_X[N+1], the points that define the intervals.
//    Interval I lies between INT_X(I) and INT_X(I+1).
//
//    Input, Td INT_V[N], the desired value of the integral of the
//    linear spline over each interval.
//
//    Output, Td DATA_X[N], DATA_Y[N], the values of the independent
...
//    Input, Td TVAL, the point at which the spline is to be evaluated.
//
//    Output, Td *YVAL, *YPVAL, the value of the spline and its first
...
//    Input, Td YDATA[NDATA], the data values.
//
//    Input, Td TVAL, the value where the spline is to
//    be evaluated.
//
//    Output, Td SPLINE_OVERHAUSER_NONUNI_VAL, the value of the
//    spline at TVAL.
//
{
  Td alpha;
...
//    be evaluated.
//
//    Output, Td SPLINE_OVERHAUSER_UNI_VAL, the value of the spline at TVAL.
//
{
  int left;
  Td *mbasis = nullptr;
...
void spline_overhauser_val ( int ndim, int ndata, Td tdata[],
  Td ydata[], Td tval, Td yval[] )
...
//    the abscissas.
//
//    Input, Td TVAL, the abscissa value at which the spline
//    is to be evaluated.  Normally, TDATA[0] <= TVAL <= T[NDATA-1], and
//    the data will be interpolated.  For TVAL outside this range,
//    extrapolation will be used.
//
//    Output, Td YVAL[NDIM], the value of the spline at TVAL.
//
{
  int i;
  int left;
...
//    Input, int N, the number of data points.  N must be at least 2.
//
//    Input, Td X[N], the strictly increasing independent
//    variable values.
//
//    Input, Td F[N], dependent variable values to be interpolated.  This
...
//
//    Output, Td D[N], the derivative values at the
...
void spline_pchip_val ( int n, Td x[], Td f[], Td d[],
  int ne, Td xe[], Td fe[] )
...
//    variable values.
//
//    Input, Td F[N], the function values.
//
//    Input, Td D[N], the derivative values.
//
//    Input, int NE, the number of evaluation points.
//
//    Input, Td XE[NE], points at which the function is to
//    be evaluated.
//
//    Output, Td FE[NE], the values of the cubic Hermite
//    function at XE.
//
{
  int i;
